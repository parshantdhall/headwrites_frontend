  ...
  parse(htmlContent, {
    replace: (domeNode) => {
      ...
      const foundedAnchorTag =
        domeNode.children &&
        domeNode.children.find(
          (child) => child.type === "tag" && child.name === "a"
        );
      if (foundedAnchorTag !== undefined) {
        return (
          <Text as="span">
            <Link
              href={
                foundedAnchorTag.attribs.href
                  ? foundedAnchorTag.attribs.href
                  : "/"
              }
            >
              <a
                rel={foundedAnchorTag.attribs?.rel || ""}
                target={foundedAnchorTag.attribs?.target || ""}
              >
                <Text color="blue.400">
                  {foundedAnchorTag.children[0].data}
                </Text>
              </a>
            </Link>
          </Text>
